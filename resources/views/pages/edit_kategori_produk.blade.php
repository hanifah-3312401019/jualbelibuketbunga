...
@section('content')
    <h1 class="text-2xl font-bold mb-4">✏️ EDIT KATEGORI PRODUK</h1>
    ...
    <form action="{{ route('kategori.update', $kategori->id) }}" method="POST" class="max-w-md">
        @csrf
        @method('PUT')
        <div class="mb-4">
            <label for="nama" class="block font-semibold mb-1">Nama Kategori</label>
            <input type="text" name="nama" id="nama" required class="border rounded px-3 py-2 w-full" value="{{ old('nama', $kategori->nama) }}">
        </div>
